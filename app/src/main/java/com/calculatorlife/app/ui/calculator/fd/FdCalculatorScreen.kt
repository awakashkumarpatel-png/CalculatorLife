...
fun FdCalculatorScreen(
    ...
    CalculatorScaffold(title = stringResource(R.string.calc_fd), onOpenMenu = onOpenMenu) { padding ->
        Column(
            modifier = Modifier
                .fillMaxSize()
                .padding(padding)
            ...
            NumberField(stringResource(R.string.fd_principal), state.principal, viewModel::onPrincipalChanged, allowNegative = false)
            NumberField(stringResource(R.string.fd_rate), state.annualRatePercent, viewModel::onRateChanged, allowNegative = false)
            NumberField(stringResource(R.string.fd_tenure_years), state.tenureYears, viewModel::onTenureChanged, allowNegative = false)
            ...
            state.result?.let { result ->
                ResultCard(
                    title = stringResource(R.string.fd_maturity_value),
                    value = result.maturityValue.setScale(2, RoundingMode.HALF_UP).toPlainString(),
                    subtitle = "${stringResource(R.string.fd_total_interest)}: ${result.totalInterest.setScale(2, RoundingMode.HALF_UP).toPlainString()}"
                )
            }
            if (state.error) {
