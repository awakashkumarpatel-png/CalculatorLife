...
fun TaxCalculatorScreen(
    ...
    CalculatorScaffold(title = stringResource(R.string.calc_tax), onOpenMenu = onOpenMenu) { padding ->
        Column(
            modifier = Modifier
                .fillMaxSize()
                .padding(padding)
            ...
            ModeSelector(
                options = listOf(stringResource(R.string.tax_mode_add), stringResource(R.string.tax_mode_remove)),
                selectedIndex = modes.indexOf(state.mode),
                onSelect = { viewModel.onModeSelected(modes[it]) }
            )

            NumberField(stringResource(R.string.tax_amount), state.amount, viewModel::onAmountChanged, allowNegative = false)
            NumberField(stringResource(R.string.tax_rate), state.ratePercent, viewModel::onRateChanged, allowNegative = false)

            state.result?.let { result ->
                ResultCard(
                    title = stringResource(R.string.tax_total_amount),
                    value = result.totalAmount.setScale(2, RoundingMode.HALF_UP).toPlainString(),
                    subtitle = "${stringResource(R.string.tax_base_amount)}: ${result.baseAmount.setScale(2, RoundingMode.HALF_UP).toPlainString()} · " +
                        "${stringResource(R.string.tax_amount_result)}: ${result.gstAmount.setScale(2, RoundingMode.HALF_UP).toPlainString()}"
                )
            }
            if (state.error) {
